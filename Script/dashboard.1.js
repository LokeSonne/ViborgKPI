...
// var dbFilter = new EmbeddedDashboard ();
...
//----------- Tilføjer filterboks for unge/alle
// var KPIfilter = new KPIComponent ();
// KPIfilter.setDimensions (12, 1);
// dbFilter.addComponent (KPIfilter);
// KPIfilter.lock();
// Promise.resolve(KPIfilter.setCaption('Vis kun unge under 30 år')).then(function (data) {
// KPIfilter.setValue(0);
// $( "div .rfKPICaption:contains('Vis kun unge under 30 år')" ).parents(".rfComponentContainer").attr('id', 'KpiYd0');
// $('#KpiYd0').find(".rfKPIValue").remove();
// //Unge/alle filter    
// $(function(){
//     $('#KpiYd0').click(function(e) 
//     {   
//         e.preventDefault();
//             if (window.filterValue === "alle"){
//                 window.filterValue = "unge";
//                 $('#KpiYd0').children().css("background-color", "#D8D8D8");
//                 loadKPIData();                
//                 loadChartDataAll();
//             }
//             else{
//                 window.filterValue = "alle";
//                 $('#KpiYd0').children().css("background-color", "none");
//                 loadKPIData();
//                 loadChartDataAll();
...
// //Filter gentages hvis vinduet resizes. Nødvendigt da resize reloader dashboard elementer
// //og de tilføjede events og classes fjernes.
...
// KPIfilter.unlock();
// dbFilter.embedTo("dbTargetFilter");
...
var series4HasData = false
    ...
    if( myNumberOfDataColumns >= 2 && arrayInput["arrayInput2"].length > 0 && arrayInput["arrayInput2"].reduce(function(a, b){ return Number(a) + Number(b)}, 0) > 0) {
        numberOfSeries = numberOfSeries + 1;
        series2HasData = true;
        // var sum = arrayInput["arrayInput2"].reduce(function(a, b){ return Number(a) + Number(b)}, 0);
    ...
    if( myNumberOfDataColumns >= 3 && arrayInput["arrayInput3"].length > 0 && arrayInput["arrayInput3"].reduce(function(a, b){ return Number(a) + Number(b)}, 0) > 0){
        numberOfSeries = numberOfSeries + 1;
        series3HasData = true;
        // var sum = arrayInput["arrayInput3"].reduce(function(a, b){ return Number(a) + Number(b)}, 0);
    ...
    if( myNumberOfDataColumns >= 4 && arrayInput["arrayInput4"].length > 0 && arrayInput["arrayInput4"].reduce(function(a, b){ return Number(a) + Number(b)}, 0) > 0) {
        numberOfSeries = numberOfSeries + 1;
        series4HasData = true;
        // var sum = arrayInput["arrayInput4"].reduce(function(a, b){ return Number(a) + Number(b)}, 0);
